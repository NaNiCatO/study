...
class Simple_animation_window(QWidget):
    def __init__(self):
        ...
        self.ineract_button = QPushButton("Pause", self)
        self.ineract_button.clicked.connect(self.interact_button)
        layout.addWidget(self.ineract_button)
    ...
    def interact_button(self):
        if self.anim_area.timer.isActive():
            ...
            self.ineract_button.setText("Resume")
        else:
            self.anim_area.timer.start()
            self.ineract_button.setText("Pause")
