...
class Enter_pos_integer(Exception):
    def __init__(self):
        messagebox.showerror(title=None, message="Please be the positive integer")        

class Enter_integer(Exception):
    def __init__(self):
        messagebox.showerror(title=None, message="Please enter the integer")

class Incorrect_format(Exception):
        def __init__(self):
            messagebox.showinfo(title="Incorrect format", message="Please enter the correct format\n '10', '(3+3)-2' and '+','-','*','/'")
...
class Income_and_Expenses(Declare_variable):
    def __init__(self, month, year, data, id = 0):
        ...
        self.manage_data = Tk()
        self.manage_data.title("Set value")
    ...
    def store_data(self,id):
        try :
            In = self.In.get().replace(" ", "")
            Ex = self.Ex.get().replace(" ", "")
            if In == "" :
                if eval(Ex) < 0:
                ...
                if id == "add_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0],self.data[day][1]+eval(Ex)]
                elif id == "subtract_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0],self.data[day][1]-eval(Ex)]
                elif id != 0:
                    self.data[id] = [self.data[id][0],eval(Ex)]
                else : 
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0],eval(Ex)]
            elif Ex == "" :
                if eval(In) < 0 :
                    raise Enter_pos_integer
                if id == "add_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0]+eval(In),self.data[day][1]]
                elif id == "subtract_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0]-eval(In),self.data[day][1]]
                elif id != 0:
                    self.data[id] = [eval(In),self.data[id][1]]
                else : 
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [eval(In),self.data[day][1]]
            else :
                if eval(In) < 0 or eval(Ex) < 0:
                    raise Enter_pos_integer
                if id == "add_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0]+eval(In),self.data[day][1]+eval(Ex)]
                elif id == "subtract_all":
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [self.data[day][0]-eval(In),self.data[day][1]-eval(Ex)]
                elif id != 0:
                    self.data[id] = [eval(In),eval(Ex)]
                else : 
                    for day in self.date.itermonthdays(self.year, self.month):
                        if day != 0 :
                            self.data[day] = [eval(In),eval(Ex)]
...
class Finance_Calendar(Income_and_Expenses,Generate_table):
    def __init__(self,month,year,data) :
        ...
        self.window.title("Click to set values")
...
class Edit_days(Income_and_Expenses,Generate_table):
    def __init__(self, month, year, data):
        Generate_table.__init__(self, month, year, data)
        self.window.title("Click to edit values")
        Label(self.info,text="Positive number = Add\nNegative number = Subtract",bg="#242424",fg="white",font=20).grid(row=0,column=0)
    ...
    def store_data(self,id):
        try :
            ...
            Ex = self.Ex.get().replace(" ", "")
            if In == "" :
                self.data[id] = [self.data[id][0], self.data[id][1] + eval(Ex)]
            elif Ex == "" :
                self.data[id] = [self.data[id][0] + eval(In), self.data[id][1]]
            else :
                self.data[id] = [self.data[id][0] + eval(In), self.data[id][1] + eval(Ex)]
            self.manage_data.destroy()
        except :
            raise Incorrect_format
...
class Edit_value(Income_and_Expenses,Declare_variable):
    def __init__(self, month, year, data):
        ...
        Button(self.window,text="Set value for all",height=2,width=16,font=3,bg="#FFFFE0",command=self.set_for_all).grid(row=0,column=0)
        Button(self.window,text="Add value for all",height=2,width=16,font=3,bg="lightgreen",command=self.add_for_all).grid(row=0,column=1)
        Button(self.window,text="Subtract value for all",height=2,width=16,font=3,bg="#FFCCCB",command=self.subtract_for_all).grid(row=0,column=2)
        Button(self.window,text="Add,Subtract \nvalue each day",height=2,width=16,font=3,bg="#bed5f7",command=self.days ).grid(row=1,column=1)
        self.window.mainloop()
    ...
    def days(self):
        self.window.destroy()
        Edit_days(self.month, self.year, self.data)
...
class Show_concentration(Generate_table):
    def __init__(self, month, year, data):
        super().__init__(month, year, data)
        self.window.title("Click to show values")
    ...
    def find_average(self,index):
        color_list = [["#befabb","#75fa6e","#2cfc21"],["#ffbaba","#ff7373","#fc3838"],["#fff4ad","#fae76e","#fade2a"]]
        if index == 2:
            value = Generate_table.find_saving(self)
            #                   110%                130%               150%
            value_list = [(value[1]*110)/100,(value[1]*130)/100,(value[1]*150)/100]
            for day in self.date.itermonthdays(self.year, self.month):
                if day != 0 :
    ...
    def Show_info(self):
        ...
        days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat','Sun']
        for i,day in enumerate(days):
            d = Label(window,text=day,height=2,width=12,font=7,bg="#555555",fg="white")
            d.grid(row=0,column=i)
        for i,day in enumerate(self.date.itermonthdays(self.year, self.month)):
            if day != 0 :
                b = Button(window,text="In = "+str(self.data[day][0])+"\nEx = "+str(self.data[day][1]),height=3,width=12,font=3,bg="#333333",fg="white")
                b.grid(row=(i//7)+1,column=i%7)
        window.mainloop()
...
class Main_Menu :
    def __init__(self) :
        ...
        Label(main,text="Finance Calendar",pady=6,font=10,bg="#333333",fg="white").pack(side = LEFT)
