...
class Income_and_Expenses(Declare_variable):
    ...
    def store_data(self,id):
        try :
            ...
            if len(In.strip().split(" ")) <= 1:
                pass
            else :
                In = str(self.format_number(In.strip().split(" ")))
            
            if len(Ex.strip().split(" ")) <= 1:
                pass
            else :
                Ex = str(self.format_number(Ex.strip().split(" ")))
            
            if In != None and Ex != None :
                if In == "" :
                    if eval(Ex) < 0:
    ...
    def format_number(self,list):
        integer = list[::2]
        op = list[1::2]
        sum = eval(integer.pop(0))
        print(integer,op,sum)
        try : 
            if len(integer) < 1 or len(op) < 1 or len(integer) != len(op):
